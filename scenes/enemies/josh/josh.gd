...
@export var attack_distance_squared: float = 200000
@export var safe_distance_squared: float = 100000
@export var attack_cooldown: float = 2

var rice_drop_scene: PackedScene = preload("res://scenes/vfx/rice_particle.tscn")
var rice_attack_scene: PackedScene = preload("res://scenes/enemies/josh/enemy_rice.tscn")
...
func _process(delta) -> void:
	...
	if activated:
		# movement mode
		if Vector2.ZERO.distance_squared_to(target.global_position - global_position) < safe_distance_squared or not $EscapeTimer.is_stopped():
		...
		elif Vector2.ZERO.distance_squared_to(target.global_position - global_position) > attack_distance_squared:
			if not $AttackTimer.is_stopped():
				$AttackTimer.stop()
	...
	if %SightCast.is_colliding() and not will_activate:
		%RunAnim.play("alert")
		%RunAnim.queue("idle")
		target = %SightCast.get_collider(0)
		will_activate = true
...
func throw_onigiri() -> void:
	var rice = rice_attack_scene.instantiate()
	rice.global_position = global_position + Vector2(0, -29)
	rice.facing = facing
	rice.velocity = ((target.global_position - global_position).normalized() + Vector2(randf_range(-0.1, 0.1), randf_range(-0.1, 0.1))) * 400
	rice.damage_amount = 40
	rice.death_message = "you were spat on by Josh"
	rice.origin = self
	add_sibling(rice)
