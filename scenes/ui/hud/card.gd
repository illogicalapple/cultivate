...
func has_card_changed(new_has_card) -> void:
	...
	mouse_filter = Control.MOUSE_FILTER_PASS if has_card else MOUSE_FILTER_IGNORE
	if has_card: card_changed(card)
...
func trigger() -> void:
	...
	tw.tween_property($Cooldown, "value", 0, config_file.get_value(card, "cooldown"))
	cooldown_tw = tw

func _on_cooldown_value_changed(value: float) -> void:
	mouse_filter = MOUSE_FILTER_STOP if value == 0 else MOUSE_FILTER_IGNORE
